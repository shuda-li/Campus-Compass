...
1. **配置 API Key** → 点击页面头部 🔑 图标，输入 DeepSeek API Key，测试连接后保存（自动写入 .env）
2. **输入主题** → 输入活动主题（如"50人的Python编程竞赛"）
3. **确认人数** → Agent 自动扩展简略主题，询问参与人数
4. **补充信息** → 可选补充时间、物资、人员等（最多追问 3 次）
5. **生成方案** → 点击"生成完整方案"，实时流式输出；支持"重新生成"
6. **迭代改进** → 方案生成后输入改进需求：
   - "把主办单位改成电竞社" → **确定性补丁**瞬间修改 organizer
   - "开幕致辞改短一点" → 关键词锚定到 activity_content[0].phase
   - "投影仪多加 2 台" → 锚定到 activity_materials[0].name
   - "活动时间改为10月9号" → **确定性补丁**秒改，无需重生成
7. **导出 Word** → 点击"一键导出完整方案"下载 .docx（纯黑文字，清晰打印）
